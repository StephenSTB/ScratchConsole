...
contract Scratch is VRFConsumerBase{
    ...
    mapping(address => uint) private AddressToRound;

    mapping(uint => PoolTotal) private PoolTotals;
    ...
    event NewRound(uint currentPool, uint totalPool, uint nextPrizePool ,uint tokenSupply, uint mintAmount, uint roundNumber);
    ...
    // Variable to hold the value of link collected to be utilized in the next round.
    uint public currentPoolTotal;
     ...
     */
    constructor(uint initialLink) public payable
        VRFConsumerBase(
        ...
        ) 
    {
        ...
        Token = new ScratchToken("ScratchToken", 18, "SCRATCH");
    ...
    */
    function newRound() public{
        ...
        uint poolLiquidity = LINK.balanceOf(address(this));

        PoolTotals[RoundNumber] = PoolTotal(poolLiquidity, Token.getTotalSupply());

        uint nextPrizeTotal = (poolLiquidity / 10000) * 2896;

        CardRound = new ScratchCardRound(nextPrizeTotal);

        MintAmount = (MintAmount / 100) * 80;

        RoundNumber++;

        emit NewRound(currentPoolTotal, poolLiquidity, nextPrizeTotal, Token.getTotalSupply(), MintAmount, RoundNumber);
    }

    // function to pay the sender their dividends from previous rounds.
    function receiveDividend() public{
        require(AddressToRound[msg.sender] < RoundNumber);
        uint i = AddressToRound[msg.sender];
        AddressToRound[msg.sender] = RoundNumber;
        for( i; i < RoundNumber; i++ ){
            uint poolPercent = ((PoolTotals[i].liquidity / 10000) * 4000);
            uint userPoolPercent = Token.balanceOf(msg.sender) / (PoolTotals[i].tokens / 1000000000000000000);
            uint dividend = (userPoolPercent / 1000000000) * (poolPercent / 1000000000);
...
contract ScratchCardRound {
    ...
    constructor(uint total) public{
        ...
        MaxBlock = block.number + 1;//576000;
